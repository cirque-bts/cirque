...
# -------------------------------------------------------------------------------------------------
# Session::Store - セッション情報の保存先に関する設定
#
# セッション情報の保存先について指定できます。
# 基本的に変更の必要性はないですが、セッション情報の保存先を変更する場合に適宜指定してください。
#
# class: Plack::Session::Storeのサブモジュール名を指定します（初期値はDBI）。
# build_option: classで指定されたモジュールの初期化オプション構築コード。
#               第1引数にCirque::Containerオブジェクトが入ってきます。
#
# ＊コメントアウトされている設定は、セッション情報の保存先にCacheを使用する例です。
# 
#    'Session::Store' => {
#        class => 'Cache',
#        build_option => sub {
